...
class AddTagViewController: UIViewController{
    ...
    @IBAction func goHistory(sender: AnyObject!) {
        ...
        if flgSave == true{
            ...
            SEGAnalytics.sharedAnalytics().track("Saved New Meal")
            ...
            SEGAnalytics.sharedAnalytics().track("Updated New Meal")
...
extension AddTagViewController: TTGTextTagCollectionViewDelegate{
    ...
    func textTagCollectionView(textTagCollectionView: TTGTextTagCollectionView!, didTapTag tagText: String!, atIndex index: UInt, selected: Bool) {
        ...
        if selected == true{
            ...
            if flgSave == true{
                SEGAnalytics.sharedAnalytics().track("Toggled Tag on in Tag Editor", properties: ["tagName": tagText, "source": "Create Meal"])
            }
            else{
                SEGAnalytics.sharedAnalytics().track("Toggled tag on in Tag Editor", properties: ["tagName": tagText, "source": "Edit Meal"])
            }
            ...
            if flgSave == true{
                SEGAnalytics.sharedAnalytics().track("Toggled tag off in Tag Editor", properties: ["tagName": tagText, "source": "Create Meal"])
            }
            else{
                SEGAnalytics.sharedAnalytics().track("Toggled tag off in Tag Editor", properties: ["tagName": tagText, "source": "Edit Meal"])
            }
